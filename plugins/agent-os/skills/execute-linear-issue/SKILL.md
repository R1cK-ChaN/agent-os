...
12. Before considering the slice complete, apply the project handbook update map: requirements for required behavior, interfaces for boundary contracts, architecture for responsibility or data-flow changes, a new ADR for applicable durable trade-offs, and `NOW.md` for the current handoff. If accepted intent changes, update its owning normative document before the corresponding code. Remove completed work from `NOW.md`; do not use it as history. Run repository verification and Codex review according to `engineering-quality.md`. Do not commit unresolved material findings merely because the review-round limit was reached.
