...
4. Do not require an issue. Use an existing issue when the user or repository supplies one, and create a new one only when explicitly requested or required by repository policy. Use the repository's branch convention; when an isolated branch is useful and no convention exists, prefer `feat/<slug>` or `fix/<slug>`.
