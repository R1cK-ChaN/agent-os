...
6. For `reviewable` or `recoverable-only` work, run the narrowest relevant checks, inspect the exact diff only for paths that passed the safety screen, commit with a scope-first subject, and push the approved branch. Never silently mix unrelated work; a cumulative branch is valid only with prior explicit user or repository authorization whose public scope is recorded durably.
