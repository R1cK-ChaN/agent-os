...
3. Resolve the durable recovery chain from the user's target repository and any supplied task or pull request: repository, default branch, remote branch, existing pull request, and latest durable commit. Prefer remote Git state over stale local state or chat history.
4. Read the root and applicable nested `AGENTS.md` files, runtime declarations, lockfiles, specifications, scripts, tests, CI workflows, and smoke documentation without writing setup files, hooks, configuration, caches, or reports into the target repository. When the target has a project handbook, follow `README.md` → `docs/INDEX.md` and then load only task-relevant requirements, interfaces, architecture sections, and ADRs. Obtain shared progress from an existing pull request or task record when one was supplied. If the handbook is absent, do not initialize it unless the user explicitly asks or repository policy requires it.
